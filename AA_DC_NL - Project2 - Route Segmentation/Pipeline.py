...
class Pipeline(object):
    ...
    def __init__(self, source_directory_images=None, source_directory_groundtruth=None, output_directory_images="augmentedData", output_directory_groundtruth="augmentedData", save_format="PNG"):
        """
        ...
        if source_directory_images is not None:
            self._populate(source_directory=source_directory_images,
                           output_directory=output_directory_images,
                           ground_truth_directory=source_directory_groundtruth,
                           ground_truth_output_directory=output_directory_groundtruth)
    ...
    def _populate(self, source_directory, output_directory, ground_truth_directory, ground_truth_output_directory):
        ...
        # Get absolute path for output
        abs_output_directory_gt = os.path.join(ground_truth_directory, ground_truth_output_directory)

        # Scan the directory that user supplied.
        self.augmentor_images_gt, self.class_labels_gt = scan(ground_truth_directory, abs_output_directory_gt)
            
        # Make output directory/directories
        if len(self.class_labels_gt) <= 1:  # This may be 0 in the case of a folder generated
            if not os.path.exists(abs_output_directory_gt):
                try:
                    os.makedirs(abs_output_directory_gt)
                except IOError:
                    print("Insufficient rights to read or write output directory (%s)" % abs_output_directory_gt)
        else:
            for class_label in self.class_labels_gt:
                if not os.path.exists(os.path.join(abs_output_directory_gt, str(class_label[0]))):
                    try:
                        os.makedirs(os.path.join(abs_output_directory_gt, str(class_label[0])))
                    except IOError:
                        print("Insufficient rights to read or write output directory (%s)" % abs_output_directory_gt)

        # Check the images, read their dimensions, and remove them if they cannot be read
        # TODO: Do not throw an error here, just remove the image and continue.
        for augmentor_image in self.augmentor_images_gt:
            try:
                with Image.open(augmentor_image.image_path) as opened_image:
                    self.distinct_dimensions.add(opened_image.size)
                    self.distinct_formats.add(opened_image.format)
                ...
                print("There is a problem with image %s in your source directory. "
                      "It is unreadable and will not be included when augmenting."
                      % augmentor_image_gt.image_path)
                self.augmentor_images_gt.remove(augmentor_image)
            
        # Finally, we will print some informational messages.

        sys.stdout.write("Initialised with %s image(s) found.\n" % len(self.augmentor_images_gt))
        sys.stdout.write("Output directory set to %s." % abs_output_directory_gt)


    def _execute(self, augmentor_image, image_number, save_to_disk=True):
        """
        ...
        if save_to_disk:
            file_name = str(image_number) + "." + self.save_format
            try:
    ...
    def image_generator(self):
        while True:
            ...
            im_index = random.randint(0, len(self.augmentor_images_gt)-1)  # Fix for issue 52.
            yield self._execute(self.augmentor_images[im_index], save_to_disk=False), \
                self.augmentor_images[im_index].class_label_int, self._execute(self.augmentor_images_gt[im_index], save_to_disk=False), \
                self.augmentor_images_gt[im_index].class_label_int
    ...
    def add_further_directory(self, new_source_directory, new_output_directory="output"):
        ...
        self._populate(source_directory=new_source_directory,
                       ...
                       ground_truth_directory=new_source_directory,
    ...
    def rotate(self, probability, max_left_rotation, max_right_rotation, negative_rotation):
        """
        ...
        if not 0 <= max_right_rotation <= 25:
            ...
            self.add_operation(RotateRange(probability=probability, max_left_rotation=ceil(max_left_rotation),
                                           max_right_rotation=ceil(max_right_rotation), neg_rot = negative_rotation))
